...
# from pms.blueprints.billing.models.invoice import Invoice
# from pms.blueprints.bet.models.bet import Bet
# from pms.blueprints.bet.models.dice import roll
...
def users():
    ...
    for i in range(0, 99):
        ...
        random_phones.append(fake.msisdn())

    random_emails.append(app.config['SEED_ADMIN_EMAIL'])
    random_emails = list(set(random_emails))
    random_phones.append(app.config['SEED_ADMIN_PHONE'])
    random_phones = list(set(random_phones))
    ...
    while True:
        ...
        if random_percent <= 0.8:
            role = 'member'
        elif random_percent <= 0.97:
            role = 'manager'
        else:
            role = 'boss'

        email = random_emails.pop()
        phone = random_phones.pop()
